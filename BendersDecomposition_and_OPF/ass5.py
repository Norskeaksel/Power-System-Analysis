...
B = buildDCY(lines, n)  # Called Y in previous assignments but called B in the formulas I'm using
...
line = 1
sub = subSolve(transCap[1], PF, PTDF, line)
# New generation
deltaP = np.zeros(n)
...
# Task 4
fprint('\nTask 4:')
fprint('Add constraint Ks + sum((model.P[i] - Pbase[i]) * dk_dp[i] for i in model.N) <= 0')
Ks = sub.obj()
rc = np.array([i[-1] for i in sub.rc.items()])[len(sub.rc) // 2:]
subCost = np.ones(n)
dk_dp = subCost - rc
model.constraints.add(Ks + sum((model.P[i] - Pbase[i]) * dk_dp[i] for i in model.N) <= 0)
