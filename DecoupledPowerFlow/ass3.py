...
originalDPF, originalIterations = DPF(lines, X, PQsch, buses, Pnr, Qnr, slackbus, allowedMissmatch, 2, maxIterations)
...
RisXDPF, RisXIterations = DPF(lines, X, PQsch, buses, Pnr, Qnr, slackbus, allowedMissmatch, 3, maxIterations)

# Task 4
fprint('Task 4 10% load increace:')
PQsch[0] *= 1.1
DPF10, iterations10 = DPF(lines, X, PQsch, buses, Pnr, Qnr, slackbus, allowedMissmatch, 4, maxIterations)

# fprint('Task 4 20% load increace:')
# PQsch[0]=-1.2
# iterations20=DPF(lines, X, PQsch, buses, Pnr, Qnr, slackbus, allowedMissmatch,4,maxIterations)
# RisXIterations=DPF(lines, X, PQsch, buses, Pnr, Qnr, slackbus, allowedMissmatch,4,maxIterations)

fprint('ASSIGNMENT INSIGHT:')
fprint(
    'Task2: The primal, dual and standard decoupled powerflow algorithms require the following amount of iterations:')
...
fprint(
    'We see that the equality between the resistance and reactance increases the required amount of iterations before the final solution is found')
fprint("Final solutions:")
cout(RisXDPF)
...
print("All done. Results saved to ResultAssignment3.txt")
